...
namespace Scheduling
{
    class CPU
    {
        ...
        public void Execute()
        {
            while (!Done)
            {
                if (RemainingTime == 0 && ActiveProcess != -1)
                {
                    RemainingTime = -1;
        ...
        private int ExecuteLine(string sLine)
        {
            ...
            {
                // yield releases the CPU without blocking the process
                // The process remains ready, scheduler should be activated
                // Move to next line to avoid infinite yield loop
                int iNextProgramCounter = ProgramCounter + 1;
                OperatingSystem.TimeoutReached();
                return iNextProgramCounter;
            }
            else
